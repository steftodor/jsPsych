...
Current version: 2.1.0. [See version history](https://github.com/jspsych/jsPsych/blob/main/packages/plugin-html-keyboard-response/CHANGELOG.md).
...
| response_start_time | numeric | null | How long the partipipant has to wakt before they can respond in milliseconds. During this time, key responses will be ignored. If the value of this parameter is null, then the participant can respond immediately. Using this parameter will cause the response listener to start after the specified time has elapsed.  Which will cause the rt value to be measured from the start of this response period, not from the start of the trial. |
| response_end_time | numeric | null | After how many milliseconds from the trial initally starting should the response period be avalilable to the participant. After this time has elapsed, the participant will not be able to respond, but the trial will continue until the trial_duration is reached. If the value of this parameter is null, then the participant can respond until the trial ends. |
